...
int sdp_opus(uint8_t *data, int bytes, int payload, int sample_rate, int channel_count, const void* extra, int extra_size)
{
	...
	int n;
	n = snprintf((char*)data, bytes, pattern, payload, payload);
	if (2 == channel_count)
