...
{
	if (bytes < 6) return 0;
	be_write_uint16(data, RTMP_EVENT_STREAM_BEGIN);
...
{
	if (bytes < 6) return 0;
	be_write_uint16(data, RTMP_EVENT_STREAM_EOF);
	be_write_uint32(data + 2, streamId);
	return 6;
}
...
{
	if (bytes < 6) return 0;
	be_write_uint16(data, RTMP_EVENT_STREAM_DRY);
	be_write_uint32(data + 2, streamId);
	return 6;
}
...
int rtmp_event_set_buffer_length(uint8_t* data, size_t bytes, uint32_t streamId, uint32_t ms)
{
	...
	be_write_uint16(data, RTMP_EVENT_SET_BUFFER_LENGTH);
	be_write_uint32(data + 2, streamId);
...
{
	if (bytes < 6) return 0;
	be_write_uint16(data, RTMP_EVENT_STREAM_IS_RECORD);
	be_write_uint32(data + 2, streamId);
	return 6;
}
...
{
	if (bytes < 6) return 0;
	be_write_uint16(data, RTMP_EVENT_PING);
...
{
	if (bytes < 6) return 0;
	be_write_uint16(data, RTMP_EVENT_PONG);
	be_write_uint32(data + 2, timstamp);
	return 6;
}
