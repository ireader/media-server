...
#define MOV_TFHD_BASE_DATA_OFFSET			0x000001
#define MOV_TFHD_SAMPLE_DESCRIPTION_INDEX	0x000002
#define MOV_TFHD_DEFAULT_DURATION			0x000008
#define MOV_TFHD_DEFAULT_SIZE				0x000010
#define MOV_TFHD_DEFAULT_FLAGS				0x000020
...
int mov_read_tfhd(struct mov_t* mov, const struct mov_box_t* box)
{
	...
	if (MOV_TFHD_BASE_DATA_OFFSET & flags)
		mov->track->tfhd.base_data_offset = file_reader_rb64(mov->fp); /* base_data_offset*/
	else if(MOV_TFHD_DEFAULT_BASE_IS_MOOF & flags)
	...
	if (MOV_TFHD_SAMPLE_DESCRIPTION_INDEX & flags)
	...
	if (MOV_TFHD_DEFAULT_DURATION & flags)
	...
	if (MOV_TFHD_DEFAULT_SIZE & flags)
	...
	if (MOV_TFHD_DEFAULT_FLAGS & flags)
	...
	if (MOV_TFHD_DURATION_IS_EMPTY & flags)
		; /* duration©\is©\empty*/
